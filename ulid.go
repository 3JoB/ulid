...
func Parse(ulid string) (id ULID, err error) {
	return id, parse(unsafeConvert.ByteSlice(ulid), false, &id)
}
...
func ParseStrict(ulid string) (id ULID, err error) {
	return id, parse(unsafeConvert.ByteSlice(ulid), true, &id)
}
...
func (id ULID) String() string {
	...
	return unsafeConvert.StringSlice(ulid)
}
...
func (id *ULID) Scan(src any) error {
	...
	case nil:
		return nil
	case string:
		return id.UnmarshalText(unsafeConvert.ByteSlice(x))
	case []byte:
